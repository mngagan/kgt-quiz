...
class App extends Component {
  ...
  componentDidMount = () => {
    ...
    // $.ajax({
    //   url:
    //     "https://api.github.com/repos/mngagan/react-quiz-working/git/trees/master?recursive=1",
      ...
      <div style={{ textAlign: "center" }}>
        {this.state.quiz.map((val, index) => {
          return (
            <button
              key={_.random(99999) + ""}
              className="e-control e-btn e-outline e-primary buttonStyle"
              onClick={() => {
                this.handleButtonClick({ id: val.id });
              }}
              id={val.id}
              disabled={!val.active}
            >
  ...
  render() {
    let remainingCount = _.filter(this.state.quiz, arg => {
      return !arg.active;
    }).length;
        ...
        <div className="row">
          <div className="title">
            ...
            <div className={"remainingCount"}>
              {" "}
              {remainingCount} of {config.count}{" "}
            </div>
            {false && (
              <div className={"time"}>
                <span id="mins">00</span> : <span id="secs">0</span>
              </div>
            )}
          </div>
        </div>
        {(config.type === "image" || config.type === "questionsInModal") &&
          this.renderButtons()}
        {this.state.showDialog && config.type === "image" && this.renderModal()}
        {config.type === "questions" && <QuestionAnswers />}
        {this.state.showDialog &&
          config.type === "questionsInModal" &&
  ...
  renderModal = () => {
    return (
      ...
      >
        ...
        <img
          src={`./images/${this.state.activeId}.png`}
          width={500}
          height={450}
          alt={"Not Found"}
  ...
  renderQuestionsModal = () => {
    return (
      <DialogComponent
        ...
        beforeClose={() => this.dialogClose()}
      >
        <div>{QuestionAnswers[0].question}</div>
      </DialogComponent>
    );
  };
}
